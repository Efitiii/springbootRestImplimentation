...
public class MinIntHeap {
    ...
    public void heapifyUp (int input) {
        int index = input-1;
    ...
    private void heapifyDown(int current) {
        ...
        while  (hasLeftChild(current)) {
            ...
            if (hasRightChild(current)) {
                if (items[getLeftChildIndex(current)] >= items[getRightChildIndex(current)]) {
                    smallerChildIndex = getRightChildIndex(current);
                }
            }
            if (items[smallerChildIndex] < items[current]) break;

            else {
                swap(smallerChildIndex, current);
            }
